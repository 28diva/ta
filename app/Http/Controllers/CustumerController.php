...
class CustumerController extends Controller
{
    ...
    public function Tes()
    {
        ...
        return view('pages.custumer-page.wisata.landing', [
            'title' => 'Home',
            'data' => $data,
        ]);
    }
    ...
    public function showSearchRekomendasi()
    {
        ...
        return view('pages.custumer-page.wisata.search', [
            'title' => 'Rekomendasi',
            'data' => $data,
    ...
    public function rekomendasiStore(Request $request)
    {
        ...
        try {
            ...
            // dd($dataAwalAlternatif, $dataCriteria, $isIncludeJarak);
            ...
            if (empty($criteriaId) && empty($subCriteriaId)) {
                if ($request->travel_category_id && $request->travel_category_id != 'All') {
                    $dataAfterFilter1 = Alternative::where('travel_category_id', $request->travel_category_id)->with('performanceRatings')->get();
                    $dataAlternatif = $dataAfterFilter1;
                }
            } else {
                if ($request->travel_category_id && $request->travel_category_id != 'All') {
                    # code...
            ...
            foreach ($dataAlternatif as $key => $alt) {
                ...
                foreach ($alt->performanceRatings as $index => $pr) {
                    ...
                    $jarakTempuhKey = strtolower(str_replace(' ', '', $criteriaJarakTempuh->name));
                    if ($isIncludeJarak && !array_key_exists($jarakTempuhKey, $arrToPush)) {
                        DB::rollBack();
                        return back()->with('error', 'Maaf, Tidak Ada Data Wisata Dengan Jarak Tempuh ' . $request->operator_jarak . ' ' . $request->value_jarak . ' Km');
                    }
    ...
    public function wisataDetail(string $id)
    {
        $item  = Alternative::find(decrypt($id));
        return view('pages.custumer-page.wisata.detail', [
            'title' => 'Home',
            'item' => $item,
        ]);
    }
    ...
    public function lastHistory(string $id)
    {
        ...
        //    'criterias' => $criterias,
        //    'alts' => $alts,
        return view('pages.custumer-page.decision.hasil', [
            'title' => 'Rekomendasi',
            'menu' => 'Rekomendasi',
            'item' => $item,
            'criterias' => $criterias,
